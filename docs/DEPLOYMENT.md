...
## Existing Testnet Migration

### Current Arbitrum Sepolia launch cutover

The reviewed launch cutover is **not registry-only**. Run
`contracts/script/MigrateArbitrumSepolia.s.sol:MigrateArbitrumSepolia` to deploy exactly three new contracts:
`VerifierRegistry`, `BountyManager`, and `ArtifactAccessController`. It reuses the current SAT token,
`TreasuryRouter`, and USDC. The old manager and access controller remain readable as archived testnet state but are not
part of the new address set.

The migration script is deliberately fail-closed. It:

- runs only on chain ID `421614`;
- defaults to the reviewed existing SAT, router, USDC, legacy manager, protocol admin, and treasury addresses;
- requires `MIGRATION_OFFICIAL_VERIFIER_ADDRESS` and `MIGRATION_VERIFIER_MINIMUM_STAKE` instead of choosing a stake;
- requires the reused router's owner, treasury, and SAT/USDC burn routing to match the reviewed state;
- requires the legacy manager's `nextBountyId` to remain `12`, every old bounty (`1..11`) to be finalized, and every
  existing submission bond to be settled; the currently unsettled bounty `9` / submission `1` therefore blocks cutover
  until it is explicitly finalized and its bond is settled outside this script;
- never finalizes an old bounty, changes the old contracts, or changes the reused router;
- keeps verification official-only, while preserving the final owner's ability to enable permissionless verification;
- stakes only when both `MIGRATION_STAKE_OFFICIAL_VERIFIER=true` and a sufficient
  `MIGRATION_OFFICIAL_VERIFIER_STAKE_AMOUNT` are supplied;
- checks the complete new configuration before transferring ownership, then makes the three ownership transfers the
  final state-changing transactions.

Do not put a private key in an environment variable or command-line argument. The script reads only the public
`MIGRATION_DEPLOYER_ADDRESS`. If the signer is currently stored as raw text, import it once through Foundry's hidden
interactive prompt into an encrypted keystore:

```bash
cast wallet import 3sat-arb-sepolia-migrator --interactive
```

The private key and keystore password are entered into hidden prompts; they do not appear in shell history, deployment
JSON, or script logs. Set the public configuration from the migration block in `contracts/.env.example`, including an
explicit stake choice, then perform a no-broadcast simulation first:

```bash
cd contracts
export MIGRATION_BROADCAST_APPROVED=true
export MIGRATION_DEPLOYER_ADDRESS=<public-deployer-address>
export MIGRATION_OFFICIAL_VERIFIER_ADDRESS=<public-verifier-address>
export MIGRATION_VERIFIER_MINIMUM_STAKE=<amount-in-token-wei>
export MIGRATION_STAKE_OFFICIAL_VERIFIER=<true-or-false>
# Required when the preceding value is true:
export MIGRATION_OFFICIAL_VERIFIER_STAKE_AMOUNT=<amount-in-token-wei>

forge script script/MigrateArbitrumSepolia.s.sol:MigrateArbitrumSepolia \
  --rpc-url "$ARBITRUM_SEPOLIA_RPC_URL" \
  --sender "$MIGRATION_DEPLOYER_ADDRESS" \
  -vvvv
```

Only after the simulation, address set, stake choice, old-bounty settlement, and ownership destination have been
reviewed should the same script be broadcast through the encrypted account:

```bash
forge script script/MigrateArbitrumSepolia.s.sol:MigrateArbitrumSepolia \
  --rpc-url "$ARBITRUM_SEPOLIA_RPC_URL" \
  --sender "$MIGRATION_DEPLOYER_ADDRESS" \
  --account 3sat-arb-sepolia-migrator \
  --broadcast \
  --verify \
  -vvvv
```

The script writes `contracts/deployments/arbitrum-sepolia-migration-421614.json`. It contains the reused and new
addresses, all economic parameters, signer nonces, and the expected transaction count (`11` without staking, `13`
with staking), but no private key. Actual transaction hashes and receipt block numbers remain in Foundry's
`contracts/broadcast/MigrateArbitrumSepolia.s.sol/421614/run-latest.json`; use the new `BountyManager` CREATE receipt
block as the indexer start block.
